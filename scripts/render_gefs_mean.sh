#!/usr/bin/env bash
# render_gefs_mean.sh — GEFS ensemble MEAN (geavg member), 0.5° pgrb2a,
# 6-hourly to f384. Structural copy of render_gfs.sh: 4 cycles/day,
# 6-hour snap, standard AWS idx byte-range fetch.
...
for offset in $(seq 0 "${CYCLES_BACK}"); do
  ...
  echo ""
  echo "==> GEFS sweep: run=${RUN_DATE}${RUN_HOUR}Z (offset -$((offset * 6))h)"
