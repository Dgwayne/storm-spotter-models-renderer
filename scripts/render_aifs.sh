#!/usr/bin/env bash
# render_aifs.sh — ECMWF AIFS (AI Forecast System) single deterministic.
# Structural copy of render_gfs.sh: 4 cycles/day, 6-hour snap, sweep of
# recent cycles with idempotent skips. Fetch path is the ECMWF open-data
# JSON index (base_url + index_format in config/products.yml), same as
# render_ecmwf.sh. Steps are 6-hourly to f360.
...
load_model_products "${MODEL}" "${CONFIG}"
...
for offset in $(seq 0 "${CYCLES_BACK}"); do
  ...
  echo ""
  echo "==> AIFS sweep: run=${RUN_DATE}${RUN_HOUR}Z (offset -$((offset * 6))h)"
