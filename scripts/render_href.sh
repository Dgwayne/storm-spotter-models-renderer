#!/usr/bin/env bash
# render_href.sh <MODEL> — sweep recent HREF ensprod cycles (00/06/12/18z) so
# new runs land on R2 as soon as NOAA publishes them.
#
# Structural copy of render_rrfs.sh, adapted for a 4-cycles/day model:
# ${MODEL} runs at 00/06/12/18Z only, so the sweep steps back in 6-hour cycles
...
FH_START=$(yq -r ".models.${MODEL}.forecast_hours_default.start // 0" "$CONFIG")
...
for offset in $(seq 0 "${CYCLES_BACK}"); do
  ...
  echo ""
  echo "==> ${MODEL} sweep: run=${RUN_DATE}${RUN_HOUR}Z (offset -$((offset * 6))h)"

  for product in $PRODUCTS; do
    for fh in $(seq "${FH_START}" "${FH_END}"); do
