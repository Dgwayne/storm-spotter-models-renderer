...
// file it should dispatch. Add more rows here + the matching cron in
...
export default {
  async scheduled(event, env, ctx) {
    ...
    const workflow = CRON_TO_WORKFLOW[event.cron] || DEFAULT_WORKFLOW;
    ctx.waitUntil(dispatch(workflow, env.GH_DISPATCH_TOKEN));
  },
  ...
  async fetch(request, env) {
    ...
    if (!Object.values(CRON_TO_WORKFLOW).includes(wf) && wf !== DEFAULT_WORKFLOW) {
